...
import { getThreatRecord } from "@/lib/claude/threats-store";
import { runReportGeneration } from "@/lib/claude/report-service";
import { writeLegalReport } from "@/lib/claude/reports-store";
import type {
    ...
    return NextResponse.json(
      {
        error: `Threat "${request.threatId}" is not in "validated" state (current: "${threat.analysisState}"). ` +
               `Run POST /api/claude/analyze first.`,
      },
      { status: 400 }
    );
  }
